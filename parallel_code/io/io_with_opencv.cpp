...
{   
    const double intensity_range = 255.0;
 
    cv::Mat imageMat = imread(imageFilePath, cv::IMREAD_UNCHANGED);
    double* imageArray = new double[imageMat.cols * imageMat.rows];
    
    if(imageMat.data)
    {
        imageMat.convertTo(imageMat, CV_64F, 1.0 / intensity_range);
        
        for(int row = 0; row < imageMat.rows; ++row)
        {
            for(int col = 0; col < imageMat.cols; ++col)
            {
                imageArray[row * imageMat.cols + col] = imageMat.at<double>(row, col);
            }
        }
    }
